...
module.exports = {
  ...
  async execute(interaction) {
    try {
      ...
      const { participants, indecis, reservistes } =
        sortUserChoices(userChoices);
      ...
      const participantFieldIndex = embed.fields.findIndex((field) =>
        field.name.includes("Participants"),
      );
      embed.fields[participantFieldIndex].name =
        `✅ Participants (${participants.length})`;
      embed.fields[participantFieldIndex].value = formatList(participants);

      const indecisFieldIndex = embed.fields.findIndex((field) =>
        field.name.includes("Indécis"),
      );
      embed.fields[indecisFieldIndex].name = `❓Indécis (${indecis.length})`;
      embed.fields[indecisFieldIndex].value = formatList(indecis);

      const reservisteFieldIndex = embed.fields.findIndex((field) =>
        field.name.includes("En réserve"),
      );
      embed.fields[reservisteFieldIndex].name =
        `🪑 En réserve (${reservistes.length})`;
      ...
      if (isIndecis === true)
        await interaction.followUp({
          content: "Vous êtes inscrit en tant qu'**Indécis** !",
          ephemeral: true,
        });
      else
        await interaction.followUp({
...
async function handleUserChoice(interaction) {
  try {
    ...
    if (!event)
      throw new Error(
        `L'événement avec l'ID ${interaction.message.id} n'existe pas`,
      );
    if (!participantChoice)
      throw new Error('Choix "Indécis" non trouvé dans la base de données');
    ...
    console.error(
      "Erreur lors du traitement du choix de l'utilisateur :",
      error,
    );
  }
}
