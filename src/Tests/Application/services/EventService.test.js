...
describe("EventService", () => {
  ...
  describe("updateEvent", () => {
    ...
    it("should throw error for invalid date format in update", async () => {
      ...
      await expect(eventService.updateEvent(invalidUpdateData)).rejects.toThrow(
        "INVALID_DATE_FORMAT",
      );
    });
  });
  ...
  describe("cancelEvent", () => {
    it("should cancel an existing event", async () => {
      ...
      expect(mockEventRepository.update).toHaveBeenCalledWith(mockEventId, {
    ...
    it("should throw error when event not found", async () => {
      ...
      await expect(
        eventService.cancelEvent("non-existent-id", mockGuild),
