                    ...
                    if (row && row.length) {
                        ...
                        for (i = 0; i < row.length; i++) {
                            ...
                            if (row[i].channel_id === message.channel.id) {
                                db.query(`SELECT COUNT(*) AS participantCount FROM members_event_choice WHERE event_id = '${eventid}' AND choice_name = 'Participant'`, function (err, count0) {

                                    let participants = count0[0].participantCount
                                    db.query(`SELECT COUNT(*) AS indécisCount FROM members_event_choice WHERE event_id = '${eventid}' AND choice_name = 'Indécis'`, function (err, count1) {

                                        let indécis = count1[0].indécisCount
                                        db.query(`SELECT COUNT(*) AS RéservisteCount FROM members_event_choice WHERE event_id = '${eventid}' AND choice_name = 'Réserviste'`, function (err, count2) {
                                        ...
                                        VALUES ('${guild_name}','${correctedTitle}','${correctedDesc}','${event_date}','${event_hour}',${participants},${indécis},${reservistes})`)
                                            ...
                                            db.query(`DELETE FROM members_event_choice WHERE event_id = ${eventid}`)
